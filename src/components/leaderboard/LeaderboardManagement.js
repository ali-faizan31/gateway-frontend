    ...
    return (
      <>
        <div data-label="Status" className="justify-content-space-between">
          <FButton title={(status === "pending" && "Pending Review") || (status === "clientAction" && "Needs Client Action") || sentenceCase(status)}></FButton>
        ...
        <FContainer>
          <FGrid className={"f-mt-1 f-mb-1"}>
            ...
            <FGridItem alignX={"end"} alignY={"end"} dir={"row"} size={[6, 12, 12]} display={"flex"}>
              <FInputText label="Search" placeholder="Leaderboard name" variant="outlined" value={query} type="search" onChange={(e) => setQuery(e.target.value)} />
          ...
          <form autoComplete="off" onSubmit={handleSubmit(onSubmit)}>
            ...
            <FGrid>
              <FGridItem alignX="end" dir={"row"} className={"f-mt-1"}>
                <FButton type="submit" title={"Update Status"} onClick={onSubmit} postfix={isSubmitting && <ClipLoader color="#fff" size={20} />}></FButton>
