...
import { BigUtils, parseCurrency,toCurrency } from './types';
...
export const doGetApproval = createAsyncThunk('approveButton/doGetApproval',
    async (payload: {userAddress: string, contractAddress: string, currency: string,networkClient:any}, ctx) => {
    ...
    if (!!allocation && allocation.data) {
		ctx.dispatch(approvableButtonSlice.actions.allocationUpdated({
			contractAddress, userAddress, currency, allocation: allocation.data.allocation }))
	}
