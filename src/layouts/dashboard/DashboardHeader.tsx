...
import { COMMUNITY_ROLE_TAG, ME_TAG, TOKEN_TAG, ORG_ROLE_TAG, tokenFRMBSCMainnet } from '../../utils/const.utils';
...
const DashboardHeader = ({ title }: any) => {
  ...
  const getTokenInformation = async () => {
    let symbol, decimals, name, balance = null;
    try {
      if (networkClient) {
      ...
      setTokenInfo({symbol, balance: balance ? balance : '0', name, decimals});
    } catch (e) {
  ...
  return (
    <>
      <FHeader showLogo={false} titleText={title}>
        {localStorageHelper.load(ME_TAG)?.role === ORG_ROLE_TAG && <FItem align="right" display={"flex"}>
          <FButton
            title="Logout"
        ...
        {localStorageHelper.load(ME_TAG)?.role !== ORG_ROLE_TAG &&
          <FItem align="right" display={"flex"} className="no-left-margin">
            <>
              {walletAddress &&
                <>
                  <FCard variant={"primary"} className={"no-left-margin custom-padding-10 custom-border-radius-4 custom-min-width-270"}>
                    <FItem display={"flex"}> 
                      <FCard variant={"primary"} className={"d-flex custom-padding-10 overflow-visible"}>
                        {tokenInfo && tokenInfo.balance } 
                        <p className='primary-color f-pl--4'>{  " " + tokenInfo.symbol}</p>
                      </FCard>
                  ...
                  <FCard variant={"secondary"} className={"no-left-margin custom-padding-10 custom-border-radius-4 custom-min-width-270"} >
                    <FItem display={"flex"}>
                      <img src="/ferrum/wallet-address.svg" />
              ...
              <MetaMaskConnector.WalletConnector
                ...
                WalletConnectViewProps={{ className: isConnected?  "no-left-margin connect-button-left-borders" : "no-left-margin" }}
              />
              <Link
